...
func Handshake(privKey *id.PrivKey, conn net.Conn) (*session.GCMSession, id.Signatory, error) {
	...
	if _, err := io.ReadFull(conn, remotePubKeyBuf[:]); err != nil {
		return nil, id.Signatory{}, fmt.Errorf("read remote pubkey: %v", err)
	}
	...
	if _, err := io.ReadFull(conn, encryptedRemoteSecretKey[:]); err != nil {
		return nil, id.Signatory{}, fmt.Errorf("read remote secret key: %v", err)
	}
	remoteSecretKey, err := ecies.ImportECDSA((*ecdsa.PrivateKey)(privKey)).Decrypt(encryptedRemoteSecretKey[:], nil, nil)
	if err != nil {
		return nil, id.Signatory{}, fmt.Errorf("decrypt remote secret key: %v", err)
	}
	...
	if err != nil {
		return nil, id.Signatory{}, fmt.Errorf("decrypt local secret key: %v", err)
	}
	if !bytes.Equal(localSecretKey[:], localSecretKeyCheck[:]) {
		return nil, id.Signatory{}, fmt.Errorf("check local secret key")
	}
	...
	gcmSession, err := session.NewGCMSession(sessionKey, self, remote)
	if err != nil {
		return nil, id.Signatory{}, fmt.Errorf("establish gcm session: %v", err)
	}
	return gcmSession, remote, nil
}
